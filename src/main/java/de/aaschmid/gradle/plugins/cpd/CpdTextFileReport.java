...
public interface CpdTextFileReport extends SingleFileReport {
    ...
    /**
     * @return if the common leading whitespace of a source code snippet should be trimmed (= {@code true}) or not (= {@code false});
     * defaults to {@link #DEFAULT_TRIM_LEADING_COMMON_SOURCE_WHITESPACE}.
     */
    @Input
    boolean getTrimLeadingCommonSourceWhitespaces();

    /**
     * @param trimLeadingCommonSourceWhitespaces set to {@code true} if the leading common whitespaces of a single source code snippet
     * should be trimmed, otherwise {@code false}; defaults to {@link #DEFAULT_TRIM_LEADING_COMMON_SOURCE_WHITESPACE}.
     */
